...
class Dataset:
    ...
    def process_dataset(self, output_dir):
        ...
        file_format = os.path.splitext(self.url)[-1][1:]
    ...
    def splitting_more_times(self, dataset_url, dataset_format, ratio, output_dir=None, times=1):
        ...
        def get_file_url(type, id, format):
            return os.path.join(output_dir, f"dataset-{id}-{type}.{format}")
        ...
        if dataset_format == DatasetFormat.CSV.value:
            ...
            while index <= times:
                ...
                train_data_file = get_file_url(index, "train", dataset_format)
                train_data.to_csv(train_data_file, index=None)

                eval_data = new_df[int(new_num * ratio):]
                eval_data_file = get_file_url(index, "eval", dataset_format)
    ...
    def get_eval_dataset(self, feature_process=None):
        data_type = "eval"
        if self.format == DatasetFormat.CSV.value:
            eval_dataset = CSVDataParse(data_type=data_type, func=feature_process)
        elif self.format == DatasetFormat.TXT.value:
            eval_dataset = TxtDataParse(data_type=data_type, func=feature_process)
        else:
            raise ValueError(f"dataset file({self.url})'s format({self.format}) is not supported.")
        eval_dataset.parse(self.eval_dataset, label=self.label)
        return eval_dataset
