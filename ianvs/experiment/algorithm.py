...
class Algorithm:
    ...
    def build(self):
        ...
        if self.paradigm == ParadigmKind.LIFELONG_LEARNING.value:
            from sedna.core.lifelong_learning import LifelongLearning
            need_kw = getfullargspec(LifelongLearning)

            module_funcs = {}
            for module in self.modules:
                if module.kind in need_kw.args:
                    func = {"method": module.name}
                    if module.hyperparameters:
                        func["param"] = json.dumps(module.hyperparameters)
                    module_funcs[module.kind] = func
            job = LifelongLearning(estimator=basemodel_object, **module_funcs)
